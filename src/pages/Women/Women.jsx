...
const Women = () => {
  ...
  const fetchData = async ({ pageParam = 1 }) => {
    const res = await axiosSquer.get(
      `/kids?category=${women}&limit=10&page=${pageParam}`,
    );
    return res.data;
  };
  ...
  const { data: kidsLength = 0 } = useQuery({
    queryKey: ["kidsLength", "wemen"],
    queryFn: async () => {
      const res = await axiosSquer.get(`/kids/length?category=${women}`);
      return res.data;
    },
  });
  ...
  //   queryKey: ["womens"],
  //   queryFn: async () => {
  //     const res = await axiosSquer.get(`/kids?category=${women}`);
  //     return res.data;
  //   },
  // });
    ...
    <div className="pt-16">
      ...
      <div className="container mx-auto py-6 flex items-center justify-between">
        <h2 className="text-2xl font-semibold">
          Women <span className="text-primary">( {kidsLength} )</span> Kids Founts
        </h2>
